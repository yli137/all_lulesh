...
static void write_compress_buffer( const char* buf, int size, int rank, int iteration, int _rand )
{
    FILE *file;
    char file_path[50];

    sprintf( file_path, "compress_buffer/%d_%d_%d", rank, iteration, _rand );
...
static int read_compress_buffer( char **buf, int rank, int iteration, int _rand )
{
    FILE *file;
    char file_path[50];
...
static char *try_isend( const void *buf, int count, MPI_Datatype type, int dest,
	       int tag, MPI_Comm comm, MPI_Request *request, int iteration )
{
    int rank;
    ...
    if( (iteration != -1) && (read_size = read_compress_buffer( &compress_buffer, rank, iteration, _rand )) != -1 ){
        MPI_Isend( compress_buffer, read_size, MPI_BYTE, dest, tag, comm, request );
        return compress_buffer;
    }

    compress_buffer = (char*)malloc( size * count );
    int output_size = compress_lz4_buffer( (const char*)buf, size*count, compress_buffer, size*count );

	if( output_size > 0 && output_size < size*count ){
        write_compress_buffer( compress_buffer, output_size, rank, iteration, _rand );
   ...
   if (planeMin | planeMax) {
      ...
      if (planeMin) {
         ...
         addr[iaddr++] = try_isend(destAddr, xferFields*sendCount, baseType,
                   ...
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg], iteration);
         ++pmsg ;
      }
      if (planeMax && doSend) {
         destAddr = &domain.commDataSend[pmsg * maxPlaneComm] ;
   ...
   if (!planeOnly) {
      if (rowMin && colMin) {
         ...
         addr[iaddr++] = try_isend(destAddr, xferFields*dz, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg], iteration);
         ++emsg ;
      }
      ...
      if (rowMin && colMin && planeMin) {
         ...
         addr[iaddr++] = try_isend(comBuf, xferFields, baseType, toRank, msgType,
                   MPI_COMM_WORLD, &domain.sendRequest[pmsg+emsg+cmsg], iteration);
         ++cmsg ;
      }
